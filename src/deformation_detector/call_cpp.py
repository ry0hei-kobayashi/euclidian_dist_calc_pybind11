...
class EuclidianCalcCPP(smach.State):
    def __init__(self):
    ...
    def execute(self, userdata):
        try:
            ...
            bf_pose = [[bf[0], bf[1], bf[2]] for bf in bf_pos]
            af_transformed = [[af[0], af[1], af[2]] for af in af_trans]

            #######################euclidian distance calc pybind-cpp version#################################
            #corr = deformation_detection.DeformationDetection(bf_pose, af_transformed)
            bf,af = deformation_detection.DeformationDetection(bf_pose, af_transformed)
...
if __name__ == '__main__':
    rospy.init_node("euclid_cpp")

    sm = smach.StateMachine(outcomes=['success','failure'])
    with sm:

        smach.StateMachine.add('Debug', EuclidianCalcCPP(),
